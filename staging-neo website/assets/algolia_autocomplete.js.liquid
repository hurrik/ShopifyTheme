(function(algolia) {
  ...
  function addPositionToHits(hits, queryID) {
    return hits.map(function(entry, i) {
    ...
    var templates = {
      empty: function displayEmptyResultSet(props) {
        return algolia.render(
          ...
          Object.assign({}, props, { translations: algolia.translations })
        );
      },
      suggestion: function displaySuggestion(hit) {
        return algolia.render(
          params.templates.hit,
          Object.assign({ _distinct: params.distinct }, hit, {
...
//          filters: "meta.BCS.BCS_nosearch:N",
       ...
       if(section === "products") { 
	   searchOpts.filters = "meta.BCS.BCS_nosearch:N";
	   }
   	
      	if(section === "collections") { 
          searchOpts.filters = "NOT meta.meta.option-group:false";
		}
  ...
  algolia.autocomplete = autocomplete;
  ...
  algolia.appendStyle(
    algolia.render(autocomplete.templates.style, {
  ...
  $(document).ready(function mainAutocomplete() {
    ...
    autocomplete.$input.each(function() {
      ...
      var suggestionClass =
        '.' + dropdown.cssClasses.prefix + '-' + dropdown.cssClasses.suggestion;
    ...
    autocomplete.$input.on('autocomplete:selected', function(obj, datum, name) {
      if (algolia.config.analytics_enabled) {
        window.aa('click', {
          ...
          position: datum._position,
          queryID: datum._queryID,
        });
      }
      if (name === 'products') {
        var addVariantId =
          !autocomplete.products.distinct && datum.objectID !== datum.id;

(function(algolia, instantsearch) {
    ...
    $(document).ready(function() {
      ...
      instant.search.addWidget(instantsearch.widgets.starRating({
      ...
      instant.search.addWidget(
        instantsearch.widgets.numericRefinementList({
          ...
          attributeName: 'inventory_quantity',
      ...
      instant.search.on('render', function() { 
        if ($('#canonical-link')){ 
          ...
          link.attr('href', window.location.origin + window.location.pathname + window.location.search);
        } 
        if($('.ais-star-rating--count').length == 0){
        	$('.customer-rating').addClass('hidden-rating')
        }
//         console.log("Rendering Done",$('.ais-star-rating--count').length);
      });
